...
function try_mul_two_ops(a::AbstractQuantumGate, b::AbstractQuantumGate)
    r = _try_mul_two_ops_impl(positions(a), op(a), positions(b), op(b))
    (r === nothing) && return nothing
    k = (length(positions(a)) >= length(positions(b))) ? positions(a) : positions(b)
    return QuantumGate(k, r)
end

function try_absorb_one_gate_by_mul!(a::Vector{<:AbstractQuantumGate}, gate::AbstractQuantumGate)
    for i in length(a):-1:1
        r = try_mul_two_ops(gate, a[i])
        if r !== nothing
            a[i] = r
            return true
        end
...
function try_absorb_by_mul_impl(a::Vector{<:AbstractQuantumGate})
    isempty(a) && return a
    L = length(a)
    b = copy(a)
    for i in L:-1:2
        item = b[i]
        br = b[1:(i-1)]
        if try_absorb_one_gate_by_mul!(br, item)
            b = [br; b[(i+1):end]]
        end
    end
    return b
...
function _fuse_gate_impl(a::QuantumCircuit)
    b = try_absorb_by_mul_impl(raw_data(a))
    r = similar(a)
    append!(r, b)
    return r
end
